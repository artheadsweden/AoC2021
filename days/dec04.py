...
def solve(values: list[str]) -> (int, int):
    values = ''.join(values)

    numbers, *boards = values.rstrip().split('\n\n')
    ...
    first = winners[0]
    last = winners[-1]
    first = first.sum_score() * first.last_number
    last = last.sum_score() * last.last_number
    return first, last


def main():
    values = read_input_as_str('../data/input_dec06.txt')
    first, last = solve(values)
    print(f'2021-12-04 Puzzle 1: {first}')
    print(f'2021-12-04 Puzzle 2: {last}')
